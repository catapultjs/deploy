...
JSON configs are validated before command execution. Use [`config:validate`](./config-validate) to check a config explicitly in local development or CI. See [JSON configuration](/guide/json-configuration) for the supported recipes, task steps and pipeline controls.
...
| [`config:validate`](./config-validate) | Validate the deploy configuration file         | Supports `--json`                     |
